...
function modelUnion(preset, profile) {
  const set = new Set([
...
function renderCard(preset) {
  ...
  const models = modelUnion(preset, profile);
  const rows = models
    .map((model) => {
      const isNew = profile.models.includes(model) && !preset.defaultModels.includes(model);
...
function bindCard(card, preset, profile) {
  ...
  card.querySelector(".save-key").addEventListener("click", async () => {
    ...
    if (res && res.success) {
      setMsg(card, res.warning || t("providerSaved"), "ok");
      keyInput.value = "";
      await loadState();
    } else {
  ...
  card.querySelector(".detect-models").addEventListener("click", async () => {
    const summary = card.querySelector(".provider-summary");
    summary.textContent = t("providerDetecting");
    const rawKey = keyInput.value.trim();
    ...
    if (!res || !res.success) {
      summary.textContent = (res && res.error) || t("providerDetectError");
      return;
    }
    ...
    const summaryText = `${t("providerNewLabel")}: ${added.length} · ${t(
      "providerObsoleteLabel",
    )}: ${removed.length} · ${t("providerTotalLabel")}: ${after.length}`;

    await loadState();
    const newCard = document.querySelector(`.provider-card[data-provider="${provider}"]`);
    if (newCard) newCard.querySelector(".provider-summary").textContent = summaryText;
  });

  card.querySelector(".add-model").addEventListener("click", async () => {
